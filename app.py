...
def add_role():
    ...
    if r_name:
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO role (r_name) VALUES (%s)", (r_name,))
        conn.commit()
        cursor.close()
        conn.close()
        flash("เพิ่มบทบาทเรียบร้อยแล้ว", "success")
...
def edit_role(role_id):
    ...
    if request.method == "POST":
        new_name = request.form.get("r_name").strip()
        if new_name:
            cursor.execute(
                "UPDATE role SET r_name = %s WHERE role_id = %s", (new_name, role_id))
            conn.commit()
            flash("แก้ไขบทบาทเรียบร้อยแล้ว", "success")
            return redirect(url_for("manage_roles"))
...
def delete_role(role_id):
    ...
    if role and role["r_name"] != "admin":
        cursor.execute("DELETE FROM role WHERE role_id = %s", (role_id,))
        conn.commit()
        flash("ลบบทบาทเรียบร้อยแล้ว", "success")
    else:
        flash("ไม่สามารถลบบทบาท admin ได้", "danger")
